...
def difference(orig: Rectangle, removed: Rectangle) -> Set[Rectangle]:
    if (
        orig[0].x > removed[1].x
        or orig[0].y > removed[1].y
        or orig[1].x < removed[0].x
        or orig[1].y < removed[0].y
    ):
        return {orig}
    res = set()
    x = orig[0].x
...
def task2(text: str):
    ...
    rectangles = {
        (
            rotate(Coordinates(max_coord // 2, -max_coord // 2)),
