...
coords = [
    ...
    for pair in [
        l.split(" -> ") for l in open("rock-data.txt", "r").read().strip().splitlines()
    ]
]
...
def lines():
    ...
    if d > 0:
        ...
        [rocks.add((n, cur[1])) if not x_y else rocks.add((cur[0], n)) for n in li]
    if d < 0:
        d = abs(d)
        # "decreasing" order
        li = range(cur[0], cur[0] + d + 1) if not x_y else range(cur[1], cur[1] + d + 1)
        [rocks.add((n, cur[1])) if not x_y else rocks.add((cur[0], n)) for n in li]
...
[rocks.add((x, gsh)) for x in range(int(ssw - ssw / 2), int(gsw + gsw / 2))]
resting = set()
move()
